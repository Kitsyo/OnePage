...
onMounted(() => {
    ...
    .then(response => {
        ...
        task.description = response.data.data.description;
        task.date_open = response.data.data.date_open;
        task.date_close = response.data.data.date_close;
    })
...
function saveTask() {
    validate().then(form => {
        ...
        if (form.valid){
            axios.get('/api/tasks/update/'+route.params.id, task)
            .then(response => {
                strError.value = ""
